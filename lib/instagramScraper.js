            ...
            while (attempts < 3 && !profileData) {
                ...
                try {
                    ...
                    if (infoRes.ok) {
                        ...
                        logger.warn({ handle, attempt: attempts }, "[InstagramCollector] web_profile_info returned 429 (Rate Limit). Pausing 120s...");
                        onProgress?.({
                            page: 0,
                            processedItems: initialStats?.processedItems || 0,
                            totalPosts: initialStats?.totalPosts || 0,
                            statusMessage: "⚠️ Instagram rate limit (429) on profile info. Resting 120s before retry (" + attempts + "/3)...",
                            rateLimited: true,
                            isComplete: false
                        });
                        await sleep(120000);
      ...
      continue;
                ...
                if (!feedRes.ok) {
                    ...
                    if (errorText.includes("checkpoint_required") || errorText.includes("accounts/suspended") || errorText.includes("login_required")) {
                        throw new Error("⚠️ Instagram Security Checkpoint: The Instagram account used for cookies has been suspended or challenged by Meta. Please open https://www.instagram.com in your browser, log in/verify it, and paste the fresh cookie string.");
                    }
