    ...
    async runBuild(handle, onProgress, initialNextMaxId = null, initialStats = null, lastScrapedDate = null) {
      ...
      continue;
                ...
                if (contentType.includes("text/html") || responseText.trim().startsWith("<!DOCTYPE") || responseText.trim().startsWith("<html")) {
                    if (responseText.includes("login") || responseText.includes("checkpoint") || responseText.includes("/accounts/login")) {
                    ...
                    logger.error({ handle, page }, "[WORKER_LIFECYCLE] WORKER_ERROR - Received HTML instead of JSON from Instagram feed endpoint.");
                    break;
                }
                let feedJson;
                try {
