...
function parseProfileFromHTML(html, handle) {
    ...
    return {
        ...
        bio: bio?.trim() || null,
        isVerified,
        accountCreatedYear: estimateAccountCreatedYear(userId, null),
    };
}
function estimateAccountCreatedYear(userId, oldestPostDate) {
    let postYear = null;
    if (oldestPostDate) {
        try {
            const y = new Date(oldestPostDate).getFullYear();
    ...
    if (userId) {
        ...
        if (!isNaN(uid) && uid > 0) {
            if (uid < 5000000) idYear = 2010;
            else if (uid < 30000000) idYear = 2011;
            else if (uid < 250000000) idYear = 2012;
            else if (uid < 800000000) idYear = 2013;
            else if (uid < 1700000000) idYear = 2014;
            else if (uid < 2600000000) idYear = 2015;
            else if (uid < 4200000000) idYear = 2016;
            else if (uid < 6800000000) idYear = 2017;
            else if (uid < 9500000000) idYear = 2018;
            else if (uid < 15000000000) idYear = 2019;
            else if (uid < 25000000000) idYear = 2020;
            else if (uid < 40000000000) idYear = 2021;
            else if (uid < 55000000000) idYear = 2022;
            else if (uid < 65000000000) idYear = 2023;
            else if (uid < 75000000000) idYear = 2024;
            else idYear = 2025;
        }
    }

    if (postYear && idYear) {
        return Math.min(postYear, idYear);
    }
    return postYear || idYear || null;
}
function calculateScaledInteraction(rawInteractionRate, handleOrId = '') {
    if (!rawInteractionRate || rawInteractionRate <= 0) return 0;
    
    // Base 50% - 99.9% logistic scaling
    const base = 50 + 49.9 * (rawInteractionRate / (rawInteractionRate + 0.65));
    
    // Deterministic unique hash offset based on creator's handle to ensure natural variance
    let hashOffset = 0;
    if (handleOrId) {
        let hash = 0;
        const str = String(handleOrId).toLowerCase();
        for (let i = 0; i < str.length; i++) {
            hash = (hash * 31 + str.charCodeAt(i)) % 10007;
        }
        // Spread between -0.45% and +0.45%
        hashOffset = ((hash % 91) - 45) / 100;
    }
    
    let result = base + hashOffset;
    if (result < 50.01) result = 50.01;
    if (result > 99.95) result = 99.95;
...
class InstagramCollector {
    ...
    getAuthContext() {
        ...
        return { cookieHeader, csrfToken, hasAuth, isFullCookie };
    }
    async getProfileInfo(handle) {
        const { cookieHeader, csrfToken, hasAuth, isFullCookie } = this.getAuthContext();
                ...
                if (infoRes.ok) {
                    ...
                    if (user) {
                        profileData = {
                            ...
                            accountCreatedYear: estimateAccountCreatedYear(user.id, null),
                        };
                        logger.info({ handle, accountCreatedYear: profileData.accountCreatedYear }, "[InstagramCollector] ✅ Profile data parsed from web_profile_info API");
                    }
                } else {
    ...
    async runBuild(handle, onProgress, initialNextMaxId = null, initialStats = null, lastScrapedDate = null) {
        const { cookieHeader, csrfToken, hasAuth } = this.getAuthContext();
        ...
        let oldestPostDate = initialStats?.oldestPostDate || null;
            ...
            while (moreAvailable && page < this.maxPages) {
                ...
                while (attempts < 5) {
                    ...
                    if (feedRes.status === 429) {
                        ...
                        const recentReels = reels.slice(0, 20);
                        const recentReelViewsTotal = recentReels.reduce((sum, r) => sum + (r.play_count ?? r.view_count ?? 0), 0);
                        const recentReelLikesTotal = recentReels.reduce((sum, r) => sum + (r.like_count ?? 0), 0);

                        const recentPosts = posts.slice(0, 12);
                        const recentPostLikesTotal = recentPosts.reduce((sum, p) => sum + (p.like_count ?? 0), 0);

                        const avgReelViews = recentReels.length > 0 
                            ? Math.round(recentReelViewsTotal / recentReels.length) 
                            : (initialStats?.averageReelViews || 0);

                        const avgReelLikes = recentReels.length > 0 
                            ? Math.round(recentReelLikesTotal / recentReels.length) 
                            : (initialStats?.averageReelLikes || 0);

                        const avgPostLikes = recentPosts.length > 0 
                            ? Math.round(recentPostLikesTotal / recentPosts.length) 
                            : (initialStats?.averagePostLikes || 0);
                        ...
                        const rawInteractionRate = profileData.followersCount > 0 && latestItemsCount > 0
                            ...
                            : 0;
                        const scaledInteraction = calculateScaledInteraction(rawInteractionRate, handle);
                        onProgress?.({
                ...
                for (const item of items) {
                    ...
                    if (dateStr && (!oldestPostDate || dateStr < oldestPostDate)) {
                        oldestPostDate = dateStr;
                    }
                    ...
                    avgReelViews = recentReels.length > 0 
                        ? Math.round(recentReelViewsTotal / recentReels.length) 
                        : (initialStats?.averageReelViews || 0);

                    avgReelLikes = recentReels.length > 0 
                        ? Math.round(recentReelLikesTotal / recentReels.length) 
                        : (initialStats?.averageReelLikes || 0);

                    avgPostLikes = recentPosts.length > 0 
                        ? Math.round(recentPostLikesTotal / recentPosts.length) 
                        : (initialStats?.averagePostLikes || 0);
                    const rawInteractionRate = profileData.followersCount > 0 && latestItemsCount > 0
                        ? ((latestItemsEngagement / latestItemsCount) / profileData.followersCount) * 100
                        : 0;
                    scaledInteraction = calculateScaledInteraction(rawInteractionRate, handle);
                    
                    const currentAccountCreatedYear = estimateAccountCreatedYear(profileData?.userId, oldestPostDate);
                    ...
                    onProgress?.({
                        ...
                        latestPostDate,
                        oldestPostDate,
                        accountCreatedYear: currentAccountCreatedYear
                    });
                }
                moreAvailable = feedJson.more_available ?? false;
                nextMaxId = feedJson.next_max_id ?? null;
        ...
        const finalAccountCreatedYear = estimateAccountCreatedYear(profileData?.userId, oldestPostDate);
        return {
            ...profileData,
            posts,
            reels,
            ...
            averageReelLikes: avgReelLikes,
            averagePostLikes: avgPostLikes,
            accountCreatedYear: finalAccountCreatedYear,
            oldestPostDate,
