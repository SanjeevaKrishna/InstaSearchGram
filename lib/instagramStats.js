...
export async function refreshInstagramStats(celebrityId, username, sessionId = null, csrfToken = null, onProgressStream = null) {
  ...
  if (!finalSessionId || !finalCsrfToken) {
  ...
  const collector = new InstagramCollector(
    finalSessionId,
    finalCsrfToken,
    250 // max pages (allows up to ~3000 posts)
  );

  // We capture the last emitted stats from onProgress
  const sessionData = await collector.runTest(username, (stats) => {
    finalStats = stats;
    if (onProgressStream) {
